...
!VR      implicit double precision (a-h,o-z),integer*8(i-n)
!VR---
      use mpi
      implicit none
!VR---
...
!VR      include "mpif.h"
      ...
      double precision :: denmin, resis, wpiwci, bete, fxsho,ave1,ave2,phib,demin2,xmax,ymax,zmax,dt&
           ,gama,dtwci                                                                    &
           ,QUOTA,wall_clock_elapsed,maximum_simulation_time,buffer_zone                  &
...
!VR         allocate ( idmap_yz(0:nymax+1,0:nzmax+1),idmap(0:nzmax),idfft(nzmax),kvec(nzlmax),jvec(nylmax))
