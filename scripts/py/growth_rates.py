...
path_list=[0.001, 0.01, 0.05, 0.1, 0.2, 0.5]
load_input('.')
spec_rho=np.zeros( (ndumps, int(nz/2), len(path_list)) )
spec_rho_max =  np.zeros( (ndumps, len(path_list)) )
k_pos =  np.zeros( (ndumps, len(path_list)) )

for k, pth in enumerate(path_list):
    path='../1d-b%s'%pth
    print (path)
    load_input(path)
    #            0      1      2    3     4     5     6       7        8
...
if 1:
    ...
    for i in range(len(path_list)):
        line,= ax1.semilogy(times, spec_rho_max[:,i], 'o', markersize=3, 
                    markerfacecolor='none', label=r'$b_0=%s$'%path_list[i])
        ...
        if not (t0==0 and t1==0):
            ...
            p=np.polyfit(times[id1:id2],np.log(spec_rho_max[id1:id2,i]),1)
    ...
    ax1 = axes[1]
    gmax=np.load('h3dtest-gmax.npy')
    b0=np.load('h3dtest-b0.npy')
    ax1.plot(path_list, growth, 'ks', markersize=7)
    ax2 = ax1.twinx()
    ax2.plot(b0[1:], gmax[1:], 'r-', label='theory')
    ax1.set_xlabel(r'$b_0$')
    ax1.set_ylabel(r'$\gamma$')
    ax2.set_ylabel(r'analytical $\gamma_{max}$')
