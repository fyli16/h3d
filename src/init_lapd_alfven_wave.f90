...
!=======================================================================
      ...
      double precision:: rxe,rye,rze,fxe,fye,fze,dtxi,dtyi,dtzi     &
                        ...
                        ,z_p_logical,r_c,q_p,dtsav,xx,yy,r,angle,amp
      ...
      real(kind=8) :: dB_B0, num_cycles, x_pos,y_pos,z_pos, B0, VA, dB0, mi
...
! single Alfven wave
               ...
               if (.true.) then
                 ...
                 r = sqrt(xx*xx+yy*yy)
                 angle=atan2(yy, xx)
                 ! plasma diameter is 0.9 of xmax
                 r = r/(xmax/2*0.9)
                 !write(*,*)"i,j,x,y,r=",i,j,xx,yy,r
                 if (r .gt. 1.) then
                   amp=0d0
                 else
                   ! Bessel function of first kind has the first zero at 3.83170597
                   amp = dB_B0*B0*BESSEL_J1(r*3.83170597)
                 endif
                 bx_ = -amp*sin(angle)*cos(kz*z_pos)
                 by_ = amp*cos(angle)*cos(kz*z_pos)
               else
                 ! infinite plasma, circularly polarized Alfven wave
                 amp=dB_B0*B0
                 bx_ =  amp*sin(kz*z_pos)
                 by_ = -amp*cos(kz*z_pos)
               endif
               ! ignore E
               ex_ = zero
               ey_ = zero
