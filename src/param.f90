...
module m_parameter  
  ...
  ! global 
  logical :: restart, uniform_load_logical, mesh_mpio 
  ...
  real*8, dimension(:,:,:), allocatable :: uniform_mesh, ex, ey, ez, bx, by, bz, jx, jy, jz, fox, foy, foz, &
  ...
  subroutine read_input
    ...
    namelist /input/ tmax, dtwci, restart, mesh_mpio, & 
      ! simulation domain
    ...
    call MPI_BCAST(mesh_mpio              ,1     ,MPI_LOGICAL          ,0,MPI_COMM_WORLD,IERR)
    ! sim. domain
  ...
  subroutine init_arrays
    ...
    allocate ( uniform_mesh(nxmax,jb-1:je+1,kb-1:ke+1), &
      ...
      jx    (nxmax,jb-1:je+1,kb-1:ke+1), jy     (nxmax,jb-1:je+1,kb-1:ke+1), jz     (nxmax,jb-1:je+1,kb-1:ke+1), &
